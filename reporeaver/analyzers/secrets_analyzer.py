...
SECRET_PATTERNS: List[SecretPattern] = [
    SecretPattern(
        ...
        re.compile(r'(?i)(?:aws|amazon)[_-]?(?:access|secret|key|id)?[_-]?(?:key|id|token|secret)\s*[:=]\s*["\']?(AKIA[0-9A-Z]{16})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:aws|amazon)[_-]?(?:secret|key)?[_-]?(?:key|access)[_-]?(?:secret|token)\s*[:=]\s*["\']?([A-Za-z0-9/+]{40})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)github[_-]?(?:token|pat|key|secret)\s*[:=]\s*["\']?(ghp_[A-Za-z0-9]{36,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)github[_-]?(?:oauth|app)[_-]?(?:token|key|secret|id)\s*[:=]\s*["\']?(gh[osu]_[A-Za-z0-9]{36,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:slack|discord)[_-]?(?:token|key|webhook|secret|bot)\s*[:=]\s*["\']?(xox[abpors]-[A-Za-z0-9-]{24,})["\'\s]'),
    ),
    SecretPattern(
        r'(?i)(?:sk-)[A-Za-z0-9_-]{20,}(?:[A-Za-z0-9_-]{20,})?',
        "OpenAI API Key", "sk- prefix — OpenAI key", Severity.HIGH, False,
        re.compile(r'(?i)(?:sk-)[A-Za-z0-9_-]{20,}(?:[A-Za-z0-9_-]{20,})?'),
    ),
    SecretPattern(
    ...
    SecretPattern(
        ...
        re.compile(r'(?i)(?:secret|token|api[_-]?key)\s*[:=]\s*["\']?([A-Za-z0-9_\-=+/]{16,64})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?:postgresql|mysql|mongodb|redis|amqp|rabbitmq)://[^@]+@'),
    ),
    SecretPattern(
        r'(?i)https?://[^:]+:[^@]+@[^\s"\']+',
    ...
    SecretPattern(
        ...
        re.compile(r'(?i)(?:sf_username|sf_password|salesforce)[_-]?(?:username|password|token)\s*[:=]\s*["\']?([^"\';\s]{4,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:twilio|sendgrid|mailgun|stripe|paypal)[_-]?(?:api[_-]?key|secret|token|sid)\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:google|gcp|firebase)[_-]?(?:api[_-]?key|secret|token|json|credentials)\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:azure|ms)[_-]?(?:api[_-]?key|secret|token|connection[_-]?string)\s*[:=]\s*["\']?([A-Za-z0-9_\-=+/]{20,})["\'\s]'),
    ),
    SecretPattern(
        r'(?i)heroku[_-]?(?:api[_-]?key|token)\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\'\s]',
        "Heroku API Key", "Heroku platform credential", Severity.HIGH, False,
        re.compile(r'(?i)heroku[_-]?(?:api[_-]?key|token)\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\'\s]'),
    ),
    SecretPattern(
        ...
        re.compile(r'(?i)(?:PASSWORD|SECRET|TOKEN|API_KEY)\s*=\s*["\']?([A-Za-z0-9_\-=+/]{16,})["\'\s]'),
    ),
]
...
def _scan_text(content: str, path: str) -> AnalyzerResult:
    ...
    for line_no, line in enumerate(lines, 1):
        ...
        for pattern in SECRET_PATTERNS:
            m = pattern.compiled.search(stripped)
            if not m:
                continue
            # If pattern requires high entropy, gate on it
            if pattern.requires_high_entropy:
                if line_entropy is None:
                    line_entropy = shannon(stripped)
            ...
            findings.append(Finding(
                path, pattern.severity, Confidence.HIGH, Category.CREDENTIAL_THEFT,
                title=f"Hardcoded {pattern.name}",
                description=f"Found {pattern.name.lower()} in source code: {safe}",
                attack_path=f"Source leaked -> {pattern.name} extracted -> unauthorized access to cloud/service",
